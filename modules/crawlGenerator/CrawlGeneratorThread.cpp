...
/**
	this function checks if the joint limits are respected and adapt the commands
	if needed.
**/
...
bool GeneratorThread::getEncoders()
{
	...
	if(PartEncoders->getEncoders(tmp_enc))
	{
		...
		for(int i=0;i<nbDOFs;i++)
		{
			// FIXME : the following risks to cause segmentation fault since
			// 	the size of encoders is defined somewhere else in the code
...
/**
	Retrieve torque values for the joints and writes them to the ``torque_file``
	file.
**/
...
/// It also sends some of its values (from y_cpgs) to the `current_state_port`.
...
bool GeneratorThread::init(yarp::os::ResourceFinder &rf)//CTmodified: init(Searchable &s)
{
	...
	{
	  fprintf(stderr, "Using the default file option (icubSim)\n");
	  robot = "icubSim";
	}
