...
class GeneratorThread : public yarp::os::RateThread
{
 ...
 private:
    ...
    int nbLIMBs; /**< Number of limbs coupled with the one controlled by this CPG */
    int *jointMapping; /**< Mapping of the joints and the states */
    double *y_cpgs; /**< Missing Description (modified in getParameters and getOtherLimbStatus) */
    ...
    // For coupling of the CPGs of each limb, the different limbs are
    // connected together. These variables store information about these
    // connections. There is also other_part_port[3] which allows communication
    // between the CrawlGeneratorModule instances.
